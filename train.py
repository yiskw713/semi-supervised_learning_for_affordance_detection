...
def main(
    ...
    for epoch in tqdm.tqdm(range(max_epoch)):
        ...
        if best_iou < mean_iou[-1]:
            best_iou = mean_iou[-1]
            torch.save(model.module.state_dict(), result_path + '/best_iou_model.prm')
            torch.save(model_d.state_dict(), result_path + '/best_iou_model_d.prm')

        if epoch%50 == 0 and epoch != 0:
            torch.save(model.module.state_dict(), result_path + '/epoch_{}_model.prm'.format(epoch))
    ...
    torch.save(model.module.state_dict(), result_path + '/final_model.prm')
